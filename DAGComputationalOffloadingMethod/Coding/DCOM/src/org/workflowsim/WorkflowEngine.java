...
public final class WorkflowEngine extends SimEntity {
    ...
    protected void processJobReturn(SimEvent ev) {
        ...
        if (job.getCloudletStatus() == Cloudlet.FAILED) {
            ...
            int newId = getJobsList().size() + getJobsSubmittedList().size();  // assign new id to failed job
            getJobsList().addAll(ReclusteringEngine.process(job, newId));   // new job created with new iD to failed job and added in getJobList()
        }
    ...
    protected void submitJobs() {
        List<Job> list = getJobsList();     // clustered jobs transfered from clusteringEngine
        Map<Integer, List> allocationList = new HashMap<>();  // initialized to map each scheduler's id with job list 

        for (int i = 0; i < getSchedulers().size(); i++) {
            List<Job> submittedList = new ArrayList<>();    // container initialize to hold joblist of each scheduler
            allocationList.put(getSchedulerId(i), submittedList);  //mapping scheduler's id with empty joblist container
        }
        int num = list.size();
        for (int i = 0; i < num; i++) {
            //at the beginning
            Job job = list.get(i);
            Log.printLine("workflowEngine job id #" + job.getCloudletId());
            //Dont use job.isFinished() it is not right
            Log.printLine("workflowEngine : getJobsReceivedList() size :" + getJobsReceivedList().size());

            //Dont use job.isFinished() it is not right
            if (!hasJobListContainsID(this.getJobsReceivedList(), job.getCloudletId())) {  // if getJobsReceivedList() container is not having the job id, continues
                List<Job> parentList = job.getParentList();
                boolean flag = true;
                // checks all parents of this job were completed
                for (Job parent : parentList) {
                    Log.printLine("number of parent jobs :" + parentList.size());
                    Log.printLine(" parent job id #" + parent.getCloudletId());
                    if (!hasJobListContainsID(this.getJobsReceivedList(), parent.getCloudletId())) {    // checks whether this container doesn't have the job's parents
                        Log.printLine("contains parent job id #" + parent.getCloudletId());
                        flag = false;
                        break;
                    }
                }
                ...
                if (flag) {
                    ...
                    Log.printLine("submitted job id #" + job.getCloudletId());
                    Log.printLine("From flag allocationList size :" + allocationList.size());
                    jobsSubmitted++;
        ...
        for (int i = 0; i < getSchedulers().size(); i++) {
            ...
            Log.printLine("Multi scheduler : submitted list size :" + size);
            if (interval > 0 && interval <= size) {
                Log.printLine("interval :" + interval);
                int index = 0;
                ...
                Log.printLine("MULTI SCHEDULER : submitted list size :" + submittedList.size());
